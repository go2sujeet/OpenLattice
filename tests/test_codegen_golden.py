...
def test_schemas_golden_example() -> None:
    _snapshot(
        REPO_ROOT / "example.lattice",
        gen_schemas,
        Path("example_schemas.py"),
    )


def test_routes_golden_example() -> None:
    _snapshot(
        REPO_ROOT / "example.lattice",
        gen_routes,
        Path("example_routes.py"),
    )


def test_app_golden_example() -> None:
    _snapshot(
        REPO_ROOT / "example.lattice",
        gen_app,
        Path("example_app.py"),
...
def test_schemas_golden_deterministic() -> None:
    """Generator must be pure: same spec in -> same bytes out, twice."""
    spec = parse_file(str(REPO_ROOT / "example.lattice"))
    first = gen_schemas(spec)
    second = gen_schemas(spec)
    assert first == second, "Schemas generator is non-deterministic"


def test_routes_golden_deterministic() -> None:
    """Generator must be pure: same spec in -> same bytes out, twice."""
    spec = parse_file(str(REPO_ROOT / "example.lattice"))
    first = gen_routes(spec)
    second = gen_routes(spec)
    assert first == second, "Routes generator is non-deterministic"


def test_app_golden_deterministic() -> None:
    """Generator must be pure: same spec in -> same bytes out, twice."""
    spec = parse_file(str(REPO_ROOT / "example.lattice"))
    first = gen_app(spec)
    second = gen_app(spec)
    assert first == second, "App generator is non-deterministic"
...
def test_routes_imports_referenced_entities_from_schemas() -> None:
    """routes.py must import entity models it references from .schemas.

    Ensures the split between schemas_gen.py (Pydantic models) and
    routes_gen.py (FastAPI router) stays wired together correctly.
    """
    src = """
...
"""
    spec = parse_string(src)
    actual = gen_routes(spec)
    assert "from .schemas import Post" in actual


def test_routes_does_not_import_unreferenced_entities() -> None:
    """Entities that no API references should not be imported into routes.py."""
    src = """
resource "lattice_entity" "post" {
  name = "Post"
  fields = {
...
resource "lattice_entity" "unused" {
  name = "Unused"
  fields = {
    id = "uuid"
  }
